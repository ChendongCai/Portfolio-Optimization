    ...
    for n in gene_weight:
        for i in range(len(gene_weight[n])):
            if gene_weight[n][i]-date.BENCH_WEIGHT.iloc[gene_order[n][i]]>0.05:
                ...
                gene_weight[n][i]=date.BENCH_WEIGHT.iloc[gene_order[n][i]]+0.04
                for k in range(len(gene_weight[n])):
                    if k!=i:
                        gene_weight[n][k]=gene_weight[n][k]+(to_be_changed-date.BENCH_WEIGHT.iloc[gene_order[n][i]]-0.04)/(pf_len-1)
    return (gene_order,gene_weight,size)
...
##create a new dataframe to contain covariance---covm
    ...
    for n in gene_order:
        ...
        if c11<0.1 and c11>0.05:
            ...
            penalty2=c11-0.1
        fitness=risks-returns+10000*penalty+100000*penalty2+turnover
...
start=time.time()
go,gw,iter_eva,final_eva=GA(50,30,0.6,0.01,1,1,1)
end=time.time()
run_time=end-start
print(run_time)    

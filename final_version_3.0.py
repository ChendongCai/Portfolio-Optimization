...
#    origin=[i for i in range(0,length)]
    ...
    for i in range(n):
        ...
        weight[i]=locals()['w'+str(i)]
        for j in order[i]:
            weight[i].append(date.BENCH_WEIGHT.iloc[j]+date.BENCH_WEIGHT.mean())#0.0005 adjusted
    return (order,weight,pool)
#n=20-100
##lim=0.03-0.035
#for i in range(5):
...
def trans1(order,weight):
    ...
    for i in range(length):
        ...
        for k in range(len(temp)):
            temp[k]=temp[k]/sum(temp)
        locals()['g_w'+str(i)]=temp    
        gene_weight[i]=locals()['g_w'+str(i)]
        size[i]=count
    return (gene_order,gene_weight,size)

def trans(order,weight):
    length=len(order)
    gene={}
    size={}
    gene_weight={}
    gene_order={}
    pf_len=rd.randint(50,60)
    for i in range(length):
        count=0
        summ=0
        locals()['g_o'+str(i)]=order[i][:pf_len]    
        gene_order[i]=locals()['g_o'+str(i)]
        temp=weight[i][:pf_len]
        for k in range(len(temp)):
            temp[k]=temp[k]/sum(temp)
        locals()['g_w'+str(i)]=temp    
        gene_weight[i]=locals()['g_w'+str(i)]
...
def createcovm(date,cov):    
    ...
    final_df = (df_original.T + df_temp).reindex(index = date.index, columns = date.index)    
    if date is d124:
...
#Error:
def evaluate(date,covm,gene_order,gene_weight,gene_weight_index,gene_alpha_index,lam):
##create a new dataframe to contain covariance---covm
    ...
    for n in gene_order:
        ...
        if c11<0.1 and c11>0.05:
            ...
            penalty2=c11-0.1
        fitness=risks-returns+100000*penalty+100000000*penalty2
...
def GA(iteration,popsize,pc,pm,date,cov,lam):
...
#initiate
    ...
    for it in range(iteration):        
        ...
        pop_gene_order,pop_gene_weight,size=trans(pop_order,pop_weight)
        goi,gwi,gai=turnto01(pop_gene_order,pop_gene_weight,covm,date)
        prob,cum_prob,eva=evaluate(date,covm,pop_gene_order,pop_gene_weight,gwi,gai,lam)
        best_gene_index=[prob[-i][0] for i in range(1,6)]
        best_order_gene=[]
        best_weight_gene=[]
        best_of=eva.min()
        ...
        for k in range(int(popsize/2)):
            ...
            if rd.random()<pc:
                p11,p21=crossover(pop_order,pop_weight,index[0],index[1],size)
...
# Apply Elitism            
        ...
        goi,gwi,gai=turnto01(pop_gene_order,pop_gene_weight,covm,date)
        prob,cum_prob,eva=evaluate(date,covm,pop_gene_order,pop_gene_weight,gwi,gai,lam)
        worst_gene_order=[prob[i][0] for i in range(5)]
        best_idx=0
        for index in worst_gene_order:
            ...
            best_idx+=1
# Elitism ends here       
    gene_order,gene_weight,size=trans(pop_order,pop_weight)
    goi,gwi,gai=turnto01(gene_order,gene_weight,covm,date)
    prob,cum_prob,final_eva=evaluate(date,covm,gene_order,gene_weight,gwi,gai,lam)
...
start=time.time()
go,gw,iter_eva,final_eva=GA(300,40,0.8,0.02,d3,cov3,1)
end=time.time()
run_time=end-start
print(run_time)    
rs,c6,c7,c8,c9,c10,c11=present(d3,cov3,go,gw,iter_eva,final_eva)
# Testing section

#%%
def run(date_file,cov_file,iteration,i):
        go,gw,iter_eva,final_eva=GA(iteration,30,0.8,0.01,date_file[i],cov_file[i],1)
        rs,c8,c10,c11=present(date_file[i],cov_file[i],go,gw,iter_eva,final_eva)

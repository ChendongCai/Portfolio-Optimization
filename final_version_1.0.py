...
#    origin=[i for i in range(0,length)]
    ...
    for i in range(n):
        ...
        weight[i]=locals()['w'+str(i)]
        for j in order[i]:
            weight[i].append(date.BENCH_WEIGHT.iloc[j])#0.0005 adjusted
    return (order,weight,pool)
#n=20-100
##lim=0.03-0.035
#for i in range(5):
...
##create a new dataframe to contain covariance---covm
    ...
    for n in gene_order:
        ...
        if c11<0.1 and c11>0.05:
            ...
            penalty2=c11-0.1
        fitness=risks-returns+1000*penalty
...
def mutation(po):
    ...
    point2=rd.randint(51,len(po)-1)#len(po)
...
start=time.time()
go,gw,iter_eva,final_eva=GA(300,30,0.8,0.01,d0,cov0,1)
end=time.time()
run_time=end-start
print(run_time)    
rs,c6,c7,c8,c9,c10,c11=present(d0,cov0,go,gw,iter_eva,final_eva)
# Testing section
